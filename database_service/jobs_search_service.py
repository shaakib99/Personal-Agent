...
class CreateJobsSearchMetaData(BaseModel):
    ...
    url: str = Field(description='url of Jobs')
    description: str = Field(None, description='Description of Jobs')
...
class UpdateJobsSearchMetaData(BaseModel):
    id: str = Field(description='Id of the user')
    url: str = Field(None, description='url of jobs')
    description: str = Field(None, description='Type of jobs')
    text: str = Field(description='can hold any string data, like complete json text for semantic search. This field does not necessarily has to match the metadata')


class JobsSearchQueryField(BaseModel):
...
class JobsSearchService:
    ...
    async def aget(self, data: Dict[str, Any]):
        ...
        filter = JobsSearchQueryField.model_validate(query.filter)
    ...
    async def acreate_one(self, metadata: Dict[str, Any]):
        data = CreateJobsSearchMetaData.model_validate(metadata)
        if data.user_email:
        ...
        if existing and len(existing.get('ids', [[]])[0]) > 0:
            return f'Similar JobsSearch already exists: {existing.get("documents")[0][0]}. No need to try again, update the record instead. Skipping insert.'
        data.id = str(uuid4())
    ...
    async def aupdate_one(self, metadata: Dict[str, Any]):
        data = UpdateJobsSearchMetaData.model_validate(metadata)
    ...
    async def get_metadata_json(self, op: str):
        if op == 'create':
            return [{'field_name': field_name, 'field_description': field_info.description} for field_name, field_info in CreateJobsSearchMetaData.model_fields.items()]
        elif op == 'update':
            return [{'field_name': field_name, 'field_description': field_info.description} for field_name, field_info in UpdateJobsSearchMetaData.model_fields.items()]
        elif op == 'get':
            ...
            for field_name, field_info in JobsSearchQueryField.model_fields.items():
